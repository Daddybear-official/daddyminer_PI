...
char identifier[128] = "daddybear_lubu_1";
    ...
    while(1){
        ...
        while(1){
            ...
            *local_hashrate = nonce/tdelta_ms*1000;
            ...
            len = sprintf(buf, "%ld,%d,Official PC Miner (v2.4),%s\n", nonce, *local_hashrate, identifier);
...
int main(){
    ...
    int n_threads; //= 48;
    puts("Initializing DaddyBearMiner v1.1.0...");
    ...
    while(1){
        SLEEP(5); // Never exactly two seconds, so we time it ourselves
        ...
        printf(" Username -> %s, id -> %s", username, identifier);
